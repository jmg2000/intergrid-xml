...
func main() {
	...
	db, err := sqlx.Connect("firebirdsql", "sysdba:masterkey@192.168.5.206:3050/database/intergrid_akos/inter_akos.gdb")
	if err != nil {
		log.Fatalln(err)
		return
	}
	...
	for _, nom := range xmlData.Nomenclatures {
		...
		if nom.Service == true {
			...
			if workid != 0 {
				...
				_, err := db.NamedExec("insert into works (id, parent, isfolder, rname, deleted, guid) values (gen_id(gen_works_id, 1), 5497, 0, :rname, 'D', :guid)",
					map[string]interface{}{
			...
			if plistid != 0 {
				...
				_, err := db.NamedExec("insert into pricelist (id, parent, isfolder, name, category, descript, catalogn, deleted, sourceid, guid) "+
					"values (gen_id(gen_pricelist_id, 1), 184792, 0, :name, 3, :desc, :catalogn, 'A', :sourceid, :guid)",
					map[string]interface{}{
			...
			if getDescription(getNomKind(nom.Kind)) != "Автомобиль" {
				...
				if wrhid != 0 {
					...
					_, err := db.NamedExec("insert into wrhpart (id, parent, isfolder, catalogn, rname, country, edism, gtdnum, deleted, guid) "+
						"values (gen_id(gen_wrhpart_id, 1), 1011733, 0, :catn, :rname, :country, :edism, :gtdnum, 'A', :guid)",
						map[string]interface{}{
							"catn":    fmt.Sprintf("%.30s", nom.Artikul),
							"rname":   fmt.Sprintf("%.100s", nom.Description),
							"country": fmt.Sprintf("%.25s", getDescription(country)),
							"edism":   fmt.Sprintf("%.6s", getDescription(unit)), // nom.BaseUnit
				...
				if plistid != 0 {
					...
					_, err := db.NamedExec("insert into pricelist (id, parent, isfolder, name, category, descript, catalogn, deleted, sourceid, guid) "+
						"values (gen_id(gen_pricelist_id, 1), 184791, 0, :name, 2, :desc, :catalogn, 'A', :sourceid, :guid)",
						map[string]interface{}{
							"name":     fmt.Sprintf("%.100s", nom.Description),
							"desc":     nom.FullName,
							"catalogn": fmt.Sprintf("%.100s", nom.Artikul),
	...
	for _, car := range xmlData.Cars {
		...
		if carid != 0 {
			...
			_, err := db.NamedExec("insert into cars (id, parent, isfolder, vin, data, marka, model, complectation, country, color, eng, kus, fednum, ouner, saledate, saler, fyear, deleted, ptsnum, gtdnum, guid)"+
				" values (gen_id(gen_cars_id, 1), 59114, 0, :vin, :data, :marka, :model, :compl, :country, :color, :eng, :kus, :fednum, :ouner, :saledate, :saler, :fyear, 'A', :ptsnum, :gtdnum, :guid)",
				map[string]interface{}{
		...
		if orderid != 0 {
			...
			if carid != 0 {
				...
				_, err := db.NamedExec("insert into orders (id, parent, isfolder, docnum, data, docsum, docnds, carnum, marka, model, "+
					...
					"values (:id, 177120, 0, :docnum, :data, :docsum, :docnds, :car, 'NISSAN', :model, "+
